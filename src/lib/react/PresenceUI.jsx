...
APP.scene.setAttribute("presence-notifications", AFRAME.utils.styleParser.stringify(initialState))
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gridAutoColumns: "1fr", gridAutoFlow: "column" }}>
        <label style={{ display: "flex", gap: "5px", alignItems: "center" }}>
          Browser:
          <input type="checkbox" onChange={(e) => (state.browser = e.target.checked)} checked={snap.browser} />
        </label>
        <label style={{ display: "flex", gap: "5px", alignItems: "center" }}>
          IFTTT:
          <input type="checkbox" onChange={(e) => (state.ifttt = e.target.checked)} checked={snap.ifttt} />
        </label>
      </div>
      {events.map((name) => (
        <label style={{ display: "flex", gap: "5px" }} key={name}>
          <input type="checkbox" onChange={(e) => (state[name] = e.target.checked)} checked={snap[name]} />
          {name}
        </label>
      ))}
    </div>
