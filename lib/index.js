...
function mount() {
  ...
  S$1(/* @__PURE__ */ v$1(ConfigPanel, null), root);
}
function ConfigPanel() {
  const snap = useSnapshot(state);
  y$2(() => {
    const validateURL = new URL("/validate", SERVER_URL$1);
  ...
  return /* @__PURE__ */ v$1("div", {
    ...
    disabled: snap.trelloTokenValid !== false,
    onClick: onClickValidate
  }, trelloValidationText));
}
const SERVER_URL$1 = "https://cisco-trello-server.herokuapp.com";
async function requestToken() {
  return new Promise((resolve) => {
    const socket = io(SERVER_URL$1);
...
const SERVER_URL = "https://cisco-trello-server.herokuapp.com";
...
AFRAME.registerComponent("trello-board", {
  ...
  renderBoard: function (board) {
    ...
    S$1(Trello({ cards, lists }), this.webLayerComponent.rootEl);
  },
  tick: function () {
...
function useFeed(href) {
  const [feed, setFeed] = l$1();
  y$2(() => {
    const url = new URL(RSS_SERVER);
    url.searchParams.set("href", href);
    fetch(url).then((res) => res.json()).then((json) => setFeed(json));
  }, [href]);
  return feed;
}
function RSSFeed({ href, count = 5 }) {
  const feed = useFeed(href);
  return /* @__PURE__ */ v$1("div", null, feed ? /* @__PURE__ */ v$1("ul", {
    class: "grid gap-4 grid-cols-2 w-[800px]"
  }, feed.items.slice(0, count).map((item, i) => /* @__PURE__ */ v$1(FeedItem, {
    key: i,
    item
  }))) : /* @__PURE__ */ v$1("p", null, "Loading..."));
}
function FeedItem({ item }) {
  item.media?.thumbnail?.url?.replace(/&amp;/g, "&");
  const onClick = () => window.open(item.link, "_blank");
  return /* @__PURE__ */ v$1("li", {
    class: "bg-white rounded h-32 relative p-4 flex flex-col",
    "xr-layer": true
  }, /* @__PURE__ */ v$1("h1", {
    dangerouslySetInnerHTML: { __html: item.title },
    class: "flex-grow text-xl font-bold"
  }), /* @__PURE__ */ v$1("p", null, formatDate(item.published)), /* @__PURE__ */ v$1("button", {
    onClick,
    "xr-layer": true,
...
AFRAME.registerComponent("rss-feed", {
  ...
  update: function () {
    S$1(v$1(RSSFeed, { href: this.data.href, count: this.data.count }, null), this.webLayerComponent.rootEl);
  },
});
